...
class I2cDriver:
    def __init__(self):
        ...
        print(self._ctrl.frequency)
        print(self._ctrl.frequency_max)
        for address in range(0x74, 0x78):
            ...
            for pixel in range(0x24, 0xB4):
                port.write([pixel, 255])

            print(f' - Starting')
            port.write([0xfd, 0x0b])
            port.write([0x0a, 1])
    ...
    def display_frame(self, index, frame):
        ...
        #     if port.read(register)[0] != velocity:
        #         port.write([register, velocity])
